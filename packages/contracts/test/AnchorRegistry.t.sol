...
contract AnchorRegistryTest is Test {
  ...
  event Anchored(
    bytes32 indexed dataHash, bytes32 indexed stepKey, address indexed actor, uint256 timestamp
  );
  ...
  function test_anchorStep_revertsOnDuplicateStepKey() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(AnchorRegistry.AlreadyAnchored.selector, stepKey));
    registry.anchorStep(dataHash, stepKey);
  }
}
